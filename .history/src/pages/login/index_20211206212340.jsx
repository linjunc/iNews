...
const Login = () => {
    ...
    const checkMention = async (_, value) => {
        if(!result)  throw new Error('More than one must be selected!');
    };
    const onFinish = async (values) => {
        /* 处理数据后，如果成功的话，进行跳转 */
        result = post_values(values)
        if(result) {
            navigate('/')
        }else{
            checkMention()
                ...
                <LoginRight>
                    <div className='opacity'>
                        ...
                        >
                            ...
                            <Form.Item
                                name="password"
                                rules={[{ required: true, message: 'Please input your Password!' },{validator:checkMention}]}
                            >
                                <Input.Password
